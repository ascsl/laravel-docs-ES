# Eloquent: Colecciones : Eloquent: Collections
...
Todos los conjuntos de resultados múltiples devueltos por Eloquent son instancias del objeto `Illuminate\Database\Eloquent\Collection`, incluidos los resultados obtenidos a través del método `get` o al que se accede a través de una relación. El objeto de colección Eloquent amplía la [colección base](/docs/{{version}}/collections) de Laravel, por lo que hereda de manera natural docenas de métodos utilizados para trabajar con el array subyacente de modelos Eloquent.
...
Por supuesto, todas las colecciones también sirven como iteradores, lo que le permite recorrerlas como si fueran simples arrays PHP:
...
Sin embargo, las colecciones son mucho más poderosas que los arrays y exponen una variedad de operaciones de asignación / reducción que pueden estar encadenadas mediante una interfaz intuitiva. Por ejemplo, eliminemos todos los modelos inactivos y recopilemos el primer nombre para cada usuario restante:
...
> {note} Si bien la mayoría de los métodos de recolección Eloquent devuelven una nueva instancia de una colección Eloquent, los métodos `pluck`,` keys`, `zip`,` collapse`, `flatten` y` flip` devuelven una [colección base](/docs/{{version}}/collections) instancia. Del mismo modo, si una operación `map` devuelve una colección que no contiene ningún modelo Eloquent, se convertirá automáticamente en una colección base.
...
### La colección base : The Base Collection

Todas las colecciones Eloquent extienden el objeto base [colección Laravel](/docs/{{version}}/collections); por lo tanto, heredan todos los métodos potentes proporcionados por la clase de colección base:
...
Si necesita utilizar un objeto `Collection` personalizado con sus propios métodos de extensión, puede anular el método `newCollection` en su modelo:
...
Una vez que haya definido un método `newCollection`, recibirá una instancia de su colección personalizada cada vez que Eloquent devuelva una instancia de `Collection` de ese modelo. Si desea utilizar una colección personalizada para cada modelo en su aplicación, debe anular el método `newCollection` en una clase de modelo base extendida por todos sus modelos.
